...
class ModelCatalogOption extends Model {
	public function addOption($data) {
		...
		if (isset($data['option_value'])) {
			foreach ($data['option_value'] as $option_value) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "option_value SET option_id = '" . (int)$option_id . "', image = '" . $this->db->escape(html_entity_decode($option_value['image'], ENT_QUOTES, 'UTF-8')) . "', price = '" . $this->db->escape($option_value['price']) . "' , sort_order = '" . (int)$option_value['sort_order'] . "'");

				$option_value_id = $this->db->getLastId();

				foreach ($option_value['option_value_description'] as $language_id => $option_value_description) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "option_value_description SET option_value_id = '" . (int)$option_value_id . "', language_id = '" . (int)$language_id . "', option_id = '" . (int)$option_id . "', name = '" . $this->db->escape($option_value_description['name']) . "', status = '1'");
				}
			}
		}
		...
		if (isset($data['option_value'])) {
			foreach ($data['option_value'] as $option_value) {
				if ($option_value['option_value_id']) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "option_value SET option_value_id = '" . (int)$option_value['option_value_id'] . "', option_id = '" . (int)$option_id . "', image = '" . $this->db->escape(html_entity_decode($option_value['image'], ENT_QUOTES, 'UTF-8')) . "',price = '" . $this->db->escape($option_value['price']) . "', sort_order = '" . (int)$option_value['sort_order'] . "'");
				} else {
					$this->db->query("INSERT INTO " . DB_PREFIX . "option_value SET option_id = '" . (int)$option_id . "', image = '" . $this->db->escape(html_entity_decode($option_value['image'], ENT_QUOTES, 'UTF-8')) . "',price = '" . $this->db->escape($option_value['price']) . "' , sort_order = '" . (int)$option_value['sort_order'] . "'");
				}
	...
	public function deleteOption($option_id) {
        ...
        public function disableoption($option_id,$disoptionparentid,$currentstatus) {
                if($currentstatus == 1)
                {
                    $status = 0;
                }
		...
		$this->db->query("UPDATE " . DB_PREFIX . "option_value_description SET status=$status WHERE option_id = '" . (int)$disoptionparentid . "' and option_value_id = '" . (int)$option_id . "'");
	...
	public function getOptionValueDescriptions($option_id) {
		...
		foreach ($option_value_query->rows as $option_value) {
			...
			foreach ($option_value_description_query->rows as $option_value_description) {
				$option_value_description_data[$option_value_description['language_id']] = array('name' => $option_value_description['name'],'status'=>$option_value_description['status']);
			}
