		...
		if (!$this->data) {
			foreach ($this->session->data['cart'] as $key => $quantity) {
				...
				$product = unserialize(base64_decode($key));
                                ...
                                $product_idcom = explode("~",$product['product_id']);
                                
                                $product_id = $product_idcom[0];
                                $custproid =  $product_idcom[1];
				...
				if (!empty($product['recurring_id'])) {
					...
					$recurring_id = 0;
                                ...
                                // echo "SELECT * FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) WHERE p.product_id = '" . (int)$product_id . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.date_available <= NOW() AND p.status = '1'";
				...
				if ($product_query->num_rows) {
					...
					if ($recurring_query->num_rows) {
						...
						$recurring = false;
                                        ...
                                        if(!empty($custproid))
                                        {
                                           // echo "SELECT frontimage,price,selectedoption FROM `" . DB_PREFIX . "customfinial_product` WHERE `id`=$custproid ";
                                            $sql = $this->db->query("SELECT c.frontimage,c.price,c.selectedoption,ovd.name,ovd.option_id,ovd.option_value_id FROM `" . DB_PREFIX . "customfinial_product` as c join " . DB_PREFIX . "option_value_description as ovd on c.`selectedoption`=ovd.option_value_id WHERE `id`=$custproid");
                                            $image = $sql->row['frontimage'];
                                            $price = $sql->row['price'];
                                            $selectedoption = $sql->row['selectedoption'];
                                            
                                            $sizequry = $this->db->query("select size from " . DB_PREFIX . "custom_measurements where customid = $custproid");
                                            if($sizequry->num_rows != 0)
                                            {
                                                $size= $sizequry->row['size'];
                                            }
                                            else 
                                            {
                                                $size = '';
                                            ...
                                            $sizequry = $this->db->query("SELECT `name` FROM `" . DB_PREFIX . "option_value_description` WHERE `option_value_id`=$selectedoption");
                                            $optionvaluename = $sizequry->row['name'];
                                            }
                                        }
                                        else
                                        {
                                             $image = "image/".$product_query->row['image'];
                                             $size = '';
                                             $optionvaluename='';
                                        }
