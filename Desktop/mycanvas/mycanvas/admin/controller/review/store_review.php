...
class ControllerReviewStoreReview extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('review/store_review', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	protected function getList() {
		...
		$json = @file_get_contents("http://vanstudio.co.ua/index.php?route=module/fb_sr_lite");
		if($json) {
		...
		$data['sort_author'] = $this->url->link('review/store_review', 'token=' . $this->session->data['token'] . '&sort=r.author' . $url, 'SSL');
		$data['sort_rating'] = $this->url->link('review/store_review', 'token=' . $this->session->data['token'] . '&sort=r.rating' . $url, 'SSL');
		$data['sort_status'] = $this->url->link('review/store_review', 'token=' . $this->session->data['token'] . '&sort=r.status' . $url, 'SSL');
		$data['sort_date_added'] = $this->url->link('review/store_review', 'token=' . $this->session->data['token'] . '&sort=r.date_added' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('review/store_review', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
	...
	protected function validateForm() {
		if (!$this->user->hasPermission('modify', 'review/store_review')) {
